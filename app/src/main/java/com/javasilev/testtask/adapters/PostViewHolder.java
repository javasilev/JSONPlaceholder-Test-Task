...
public class PostViewHolder extends SimpleViewHolder<Post> implements Observer<Post> {
    ...
    @Override
    public void bind(final Post post, CollectionAdapter adapter) {
        mAdapter = adapter;
        mCaptionTextView.setText(R.string.label_posts);
        mNumberEditText.setText(String.valueOf(post.getId()));
        mTextBodyTextView.setText(post.getBody());
        mGetButton.setOnClickListener(v -> {
            int id = Integer.parseInt(mNumberEditText.getText().toString());
            if (id == 0 || id > MAX_ID) {
                Toast.makeText(mContext, mContext.getString(R.string.text_exceeding_warning, "post", MAX_ID), Toast.LENGTH_SHORT).show();
                return;
            }
            mGetButton.setEnabled(false);
            mProgressFrameLayout.setVisibility(View.VISIBLE);
            TypicodeController.getPost(mNumberEditText.getText().toString(), PostViewHolder.this);
        });
    }
    ...
    public void onNext(Post post) {
        mNumberEditText.setText(String.valueOf(post.getId()));
        mTextBodyTextView.setText(post.getBody());
        mAdapter.updateItem(getAdapterPosition(), post);
    }
